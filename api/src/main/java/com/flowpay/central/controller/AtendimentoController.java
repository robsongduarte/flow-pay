...
public class AtendimentoController {
    ...
    public ResponseEntity<List<AtendimentoResponse>> listar(
            ...
            @RequestParam(required = false, name = "time") TimeAtendimento time,
            @RequestParam(required = false, name = "timeAtendimento") TimeAtendimento timeAtendimento
    ) {
        TimeAtendimento filtroTime = timeAtendimento != null ? timeAtendimento : time;
        return ResponseEntity.ok(atendimentoService.listar(status, filtroTime));
    }
